...
**Run:** 2026-09-01 01:03 UTC · **checks:** 265 passed, 0 failed, 1 not applicable
...
> **Whisper — Perception** — 5 dice (INT +4, perception +2, environment: light 1 → light (-1) -1) [6 3 5 4 3] → 2 hits
...
> **Initiative** — Torque 8+5+5 = **18** · Whisper 8+1 = **9** · Sparrow 11+2+1 = **14** · Ganger-1 · Fatima Benali 8+5 = **13** · Ganger-2 · Dmitri Volkov 6+5 = **11** · Ganger-3 · Noor Haddad 8+2 = **10** · Ganger-4 · Consuelo Ibarra 7+5 = **12**
...
> **Torque — Hammer, shot 1** — 11 dice [4 2 6 6 4 1 4 2 3 2 6] → 3 hits — Tie — the defense holds.

> **Torque — Hammer, single shot** — 11 dice at 9.5 m [4 5 1 6 6 5 4 1 3 3 4] → 4 hits vs defence 7 [6 6 1 4 2 6 2] → 3 hits → 1 net

> **Ganger-1 — soak** — 14 dice (BOD + armour) [5 1 5 3 6 2 6 4 3 6 6 5 1 2] → 7 hits → **2 boxes** of stun
...
> **Torque — Push the Limit** — 12+3 dice [5 5 3 6 1 4 5 1 3 2 3 5 4 1 5] → 6 hits · rule of six: 3

> **Ganger-1 — the receipt** — physical 4/10, wound modifier -1, defence 7 → 6

Sparrow spends a point of Edge to get in front of the whole shed — the tracker moves her to the top of the pass on the spot (4 → 9) and gives her the action back. Torque blitzes hers: five dice instead of two, [5 2 1 3 1], initiative 20. Both points come off the sheets and both spends say so in the log.
...
> **Whisper — Neural Spike, Force 4** — 13 dice [4 1 4 1 4 1 2 4 6 5 4 2 3] → 2 hits vs WIL [3 2 6 4] → 1 hits → 5 Stun

> **Whisper — Drain** — 11 dice against DV 2 [1 3 4 3 1 4 4 4 1 4 2] → 0 hits → 2 Stun to the caster
...
> **Ganger-3 · Noor Haddad — Cut-down dock gun at Torque** — 5 dice [4 1 1 2 2] → 0 hits vs Torque's defence [4 6 2 4 5 3 4] → 2 hits

> **Ganger-4 · Consuelo Ibarra — Cut-down dock gun at Torque** — 7 dice [6 5 6 4 4 4 5] → 4 hits vs Torque's defence [4 1 4 4 2 4 2] → 0 hits
...
**Recap headlines:** 21 rolls at the table · best result: 6 hits · 1 critical glitch

The GM closes the session. 35 rolls are on the record with their pools, their receipts and who could see them — the copilot's three-card exchanges among them — and the recap is written, edited and posted before anyone has found their coat.
...
| 10 | the QR encodes the SPA join screen, never the API endpoint (LIVE-3) | `a URL ending /join/<code>, with no /api/join/ in it` | `http://192.168.4.35:55350/join/YDHJ6YC9` | yes |
| 11 | …and it is a real scannable image | `a base64 PNG data URL` | `data:image/png;base64,… (4234 chars)` | yes |
| 12 | GET /join/:code is not an API route any more — no token in the response | `no "token" anywhere in the body` | `404 · {"error":{"code":"not_found","message":"route GET /join/YDHJ` | yes |
...
| 18 | …and it too points at the join screen | `a URL ending /join/<code>` | `http://192.168.4.35:55350/join/NMFZAZN5` | yes |
| 19 | redeeming it mints a gm device | `gm` | `gm` | yes |
| 20 | …bound to the SAME GM identity, not a new user | `effbcdec-8a25-4362-8363-6710e84b85bc` | `effbcdec-8a25-4362-8363-6710e84b85bc` | yes |
...
| 27 | the GM pairs a second device to the same identity (FR1.1) | `a new device token bound to the same user` | `device a9cea89a… user unchanged` | yes |
...
| 49 | …on the template row itself, not in a join table | `0329118e-2bb1-4477-bfe9-ad731aff553a` | `0329118e-2bb1-4477-bfe9-ad731aff553a` | yes |
...
| 68 | revealing a hidden token arrives as token.added | `903a57ca-7fbb-4180-9a88-c0d8f7d82545` | `903a57ca-7fbb-4180-9a88-c0d8f7d82545` | yes |
...
| 125 | the highest Initiative Score acts first | `9479b71b-dd91-4b14-9b9d-2e6411c832b1` | `9479b71b-dd91-4b14-9b9d-2e6411c832b1` | yes |
...
| 130 | Torque acts twice in turn 1 | `still above 0 after −10` | `18 → 8` | yes |
...
| 136 | boxes are modified DV minus soak hits | `2` | `2` | yes |
| 137 | nothing is written until the GM commits the card | `true` | `true` | yes |
| 138 | the boxes land on the right monitor | `2` | `2` | yes |
...
| 141 | every one of them is behind the screen and stamped with the chain | `visibility gm, chainId 1d7111bc-3a63-46c5-9d9e-036c230eccea` | `attack:gm defense:gm soak:gm` | yes |
| 142 | …and the faces on the record are the faces on the card | `the attack card and its stored row agree, die for die` | `[4,5,1,6,6,5,4,1,3,3,4]` | yes |
...
| 147 | …and every six rolls again (Rule of Six) | `at least 1 exploded dice` | `3` | yes |
| 148 | every copilot roll is stamped with the running session | `6 rolls carrying session e6a88d14…` | `6/6` | yes |
...
| 151 | the ganger's wound modifier recomputes from the filled boxes | `-1` | `-1` | yes |
| 152 | and his next defence pool is that much smaller | `7 -1 = 6` | `7 → 6` | yes |
| 153 | Seize the Initiative puts the actor above everyone still in the pass | `strictly above 8` | `4 → 9 (beat 8)` | yes |
...
| 158 | …and the score is base + those five dice + the wound modifier | `8 + 5+2+1+3+1 0 = 20` | `20 (tracker says 20)` | yes |
...
| 161 | every spend announces itself, loudly, by name (FR2.3) | `log lines naming Seize the Initiative and Blitz, with the Edge left` | `Sparrow spends 1 Edge — Seize the Initiative: initiative 4 → 9, ahead of 8 (4/5 left) \| Torque spends 1 Edge — Blitz: 5d6 [5, 2, 1, 3, 1] → initiative 20 (2/3 …` | yes |
...
| 169 | the spike lands as Stun | `5` | `5` | yes |
| 170 | a Drain resistance roll follows the cast | `threshold 2` | `2` | yes |
| 171 | the unresisted margin lands on the caster's Stun track | `2` | `2` | yes |
...
| 174 | damage in the tracker mirrors to the owner's sheet — and only the owner | `sheet.updated at gm_owner visibility on Torque's phone` | `gm_owner · physical 0/11 stun 4` | yes |
...
| 179 | the suggestion is logged GM-only, never acted on | `a gm-visibility log line` | `2 line(s), visibility gm` | yes |
...
| 188 | …and the phones can see her: a summoned spirit is not hidden opposition | `Ash-of-Kettles on the player roster` | `Torque, Whisper, Sparrow, Ash-of-Kettles, Ganger-1 · Fatima Benali` | yes |
...
| | **5b · The shared log** | | | |
| 199 | the log is not empty before the beat starts | `events already on the record` | `161 events, newest id 161` | yes |
| 200 | the roll is on the immutable record | `present in GET /api/campaigns/:id/rolls` | `present` | yes |
| 201 | …and on the shared log the GM reads back | `a roll.created event carrying the roll's id` | `event 162` | yes |
| 202 | …so the log has actually grown since the beat began | `an event id above 161` | `162` | yes |
...
| 204 | …and it reached the live socket as the same event id | `frame id 162` | `162` | yes |
| 205 | table talk from a phone appends to the same log | `event 163 readable back` | `present` | yes |
| 206 | the GM advances the in-game clock | `2076-06-13` | `2076-06-13` | yes |
| 207 | …and the log carries the clock tick as table-visible history (§11) | `a clock.advanced event naming the new date` | `event 164` | yes |
...
| 234 | …whose numbers came off the log, not out of the model (D13) | `the roll tally and the award lines assembled server-side` | `21 public rolls · 3 award line(s) in the body` | yes |
...
| 238 | …the session it was actually about | `e6a88d14-50d6-4746-99dd-f31242560005` | `e6a88d14-50d6-4746-99dd-f31242560005` | yes |
...
| 243 | the session log counted the whole night, copilot dice included | `every persisted roll accounted for by the session` | `35 of 35 persisted rolls, 0 glitches` | yes |
...
| 257 | …and it can still say since when, so the number reads honestly | `the same first-counted timestamp` | `2026-09-01T01:03:35.967Z` | yes |
...
| 265 | the spirit’s spent services and the reagent tin came back too (FR8.3/8.4) | `Ash-of-Kettles at 0 services, 4 drams on the shelf` | `0 service(s) · {"96cdbbdc-ec0a-40dd-92a3-579b94fbb169":4}` | yes |
| 266 | and the event log takes the next id, not one already used | `an id above 191` | `event 193` | yes |

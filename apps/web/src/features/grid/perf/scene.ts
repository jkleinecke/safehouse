 ...
 *
 * Pure data — no DOM, no pixi, no network. `apps/web/e2e/perf.spec.ts` POSTs it
 * through the real REST routes and then drives the real renderer over it, and
 * the unit tests next door pin the properties the measurement depends on
 * (exactly 60 tokens, none overlapping enough to swallow a drag, fog that
 * actually occludes). A benchmark whose scene drifts is a benchmark whose
 * numbers cannot be compared across runs, so every coordinate here comes out of
 * a seeded PRNG and nothing is random at run time.
 *
...
/**
 * The token the drag step grabs: the one nearest the middle of the map, so a
 * fitted camera always has it comfortably on screen and the drag path stays
 * inside the canvas whichever way the viewport is shaped.
 */
export function dragTargetIndex(tokens: readonly PerfTokenSpec[]): number {
  const mid = { x: PERF_GRID.cols / 2, y: PERF_GRID.rows / 2 };
  let best = 0;
  let bestDist = Infinity;
  tokens.forEach((token, i) => {
    // Big tokens are easier to hit; nothing else about the choice matters.
    const d = Math.hypot(token.x - mid.x, token.y - mid.y) - token.size * 0.25;
    if (d < bestDist) {
      bestDist = d;
      best = i;
    }

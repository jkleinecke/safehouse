...
export type SheetTab = 'skills' | 'combat' | 'magic' | 'gear' | 'background' | 'ledger';
 ...
 * attributes"). SheetV1 carries no tradition field, so the pick is remembered
 * per character for the session and shown in the cast dialog.
 * INTEGRATION: move to a first-class `tradition` on the sheet if one lands.
 */

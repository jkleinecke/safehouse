...
- **FR3.7** Advancement (Phase 5, **built** — `docs/CHARGEN.md` §9): guided karma spends (raise attribute/skill, new spell, etc.) generating correct costs from the standard cost tables (e.g., attributes at new rating × 5, active skills at new rating × 2, skill groups at new rating × 5), editable before submit, landing as pending ledger entries plus a sheet mutation on approval.
- **FR3.8** Character revisions: every accepted sheet mutation snapshots; owner/GM can view history and roll back.
- **FR3.9** Native character creation (Phase 6, **built** — plan and status in `docs/CHARGEN.md`): the Priority system with Sum-to-Ten as a variant, a nine-step guided walkthrough, GM approval, and the creation tables as numeric mechanics data (§14.3). Karma-gen and Life Modules stay out of scope until asked for. Chummer import stays as a second way in.
...
3. **The NPC generator ships no book stat blocks.** Its stat output is parameterized sampling from GM-authored template ranges; the flavor tables it ships (names, quirks, motivations) are our own original writing. `[D10]`
   The character builder (FR3.9, `docs/CHARGEN.md` §8.1) holds the creation tables it must enforce — the priority table, metatype base/max, the skill list with its attributes and groups, advancement Karma costs, implant grade and focus multipliers, lifestyle costs, level presets — as **numeric mechanics data**: numbers, ids and `{ book, page }` refs, no descriptions and no rules text, on the same line as the dice engine's formulas (§10.2). Quality, gear and spell text stays on the GM's own page; the builder's messages are our own words.
...
| **P5 — Deep SR5** | **Magic toolkit first (M8)** — casting/drain flow, sustained spells, spirits, foci, adept powers (party: mage/shaman + adept, per 0.6); ~~advancement editor (FR3.7)~~ **shipped**; template library polish; **Fixer: stagecraft — mood-tagged music library + scene matching (FR12.10)** | A magic-heavy run needs no side spreadsheets; the sam's recoil/ammo/'ware math is invisible; the room sounds like the scene |
| **P6 — Stretch & on-demand** | Matrix toolkit (M7 — waits for a decker), token vision + dynamic lighting (FR9.16), Matrix overlay (FR9.17), ~~native priority char-gen (FR3.9)~~ **shipped**, **image-gen adapter: map art + token portraits**, PWA offline sheet cache, campaign export/archive | Only if the table comes to want them |
...
| D5 | Chummer import is the character pipeline; native char-gen (FR3.9) confirmed as a real P6 goal, not a maybe — **built 2026-09-17; Chummer import stays alongside it** | **Accepted 0.6, delivered** |
...
5. ~~Native character creation eventually, or Chummer-forever?~~ **Resolved 0.6: native eventually — delivered 2026-09-17.** FR3.9 ships (`docs/CHARGEN.md` §9); Chummer import remains as a second way in.

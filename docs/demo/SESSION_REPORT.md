...
**Run:** 2026-08-29 19:44 UTC · **checks:** 265 passed, 0 failed, 1 not applicable
...
> **Whisper — Perception** — 5 dice (INT +4, perception +2, environment: light 1 → light (-1) -1) [3 4 4 2 5] → 1 hits
...
> **Initiative** — Torque 8+3+5 = **16** · Whisper 8+2 = **10** · Sparrow 11+1+6 = **18** · Ganger-1 · Fatima Benali 8+6 = **14** · Ganger-2 · Dmitri Volkov 6+4 = **10** · Ganger-3 · Noor Haddad 8+5 = **13** · Ganger-4 · Consuelo Ibarra 7+2 = **9**
...
> **Torque — Hammer, shot 1** — 11 dice [3 1 1 5 6 1 2 1 1 3 5] → 3 hits — Tie — the defense holds.

> **Torque — Hammer, single shot** — 11 dice at 9.5 m [4 2 6 6 4 1 5 1 3 5 2] → 4 hits vs defence 7 [6 4 3 2 1 2 1] → 1 hits → 3 net

> **Ganger-1 — soak** — 14 dice (BOD + armour) [2 2 5 2 3 2 1 2 4 1 2 2 2 1] → 1 hits → **10 boxes** of physical
...
> **Torque — Push the Limit** — 12+3 dice [6 4 5 6 2 4 1 6 6 3 1 4 4 6 6] → 8 hits · rule of six: 1 1 4 2 2 5

> **Ganger-1 — the receipt** — physical 10/10, wound modifier -3, defence 7 → 4

Sparrow spends a point of Edge to get in front of the whole shed — the tracker moves her to the top of the pass on the spot (8 → 8) and gives her the action back. Torque blitzes hers: five dice instead of two, [3 4 3 1 3], initiative 22. Both points come off the sheets and both spends say so in the log.
...
> **Whisper — Neural Spike, Force 4** — 13 dice [2 6 5 2 5 3 2 1 5 3 3 1 6] → 5 hits (4 inside the limit) vs WIL [4 2 3 3] → 0 hits → 8 Stun

> **Whisper — Drain** — 11 dice against DV 2 [4 4 5 5 1 5 4 3 2 1 4] → 3 hits → 0 Stun to the caster
...
> **Ganger-3 · Noor Haddad — Cut-down dock gun at Torque** — 4 dice [1 4 2 6] → 1 hits vs Torque's defence [3 1 5 2 5 4 4] → 2 hits

> **Ganger-4 · Consuelo Ibarra — Cut-down dock gun at Torque** — 7 dice [3 2 6 5 2 3 5] → 3 hits vs Torque's defence [1 5 3 4 3 4 3] → 1 hits
...
The crate moves, and the GM draws from *Docklands complications*: “The shed loses grid power. Battery lighting comes up red, the maglocks fail shut, and the roller door stays where it is.”

**Recap headlines:** 16 rolls at the table · best result: 8 hits · 1 critical glitch

The GM closes the session. 31 rolls are on the record with their pools, their receipts and who could see them — the copilot's three-card exchanges among them — and the recap is written, edited and posted before anyone has found their coat.
...
| 10 | the QR encodes the SPA join screen, never the API endpoint (LIVE-3) | `a URL ending /join/<code>, with no /api/join/ in it` | `http://192.168.4.26:50284/join/WQN8GD5Q` | yes |
| 11 | …and it is a real scannable image | `a base64 PNG data URL` | `data:image/png;base64,… (4206 chars)` | yes |
| 12 | GET /join/:code is not an API route any more — no token in the response | `no "token" anywhere in the body` | `404 · {"error":{"code":"not_found","message":"route GET /join/WQN8` | yes |
...
| 18 | …and it too points at the join screen | `a URL ending /join/<code>` | `http://192.168.4.26:50284/join/D86JKYJ4` | yes |
| 19 | redeeming it mints a gm device | `gm` | `gm` | yes |
| 20 | …bound to the SAME GM identity, not a new user | `507fea95-7a2b-4af4-95b8-69c66ede5453` | `507fea95-7a2b-4af4-95b8-69c66ede5453` | yes |
...
| 27 | the GM pairs a second device to the same identity (FR1.1) | `a new device token bound to the same user` | `device b7bb2b50… user unchanged` | yes |
...
| 49 | …on the template row itself, not in a join table | `0dbab770-fc61-4e8a-91d6-35235acddbf3` | `0dbab770-fc61-4e8a-91d6-35235acddbf3` | yes |
...
| 68 | revealing a hidden token arrives as token.added | `0f64ebca-84ce-4457-9a71-b2ef58bd3ad7` | `0f64ebca-84ce-4457-9a71-b2ef58bd3ad7` | yes |
...
| 125 | the highest Initiative Score acts first | `ba3105a3-c5e6-4fd1-b27c-85605a3d09a3` | `ba3105a3-c5e6-4fd1-b27c-85605a3d09a3` | yes |
...
| 136 | boxes are modified DV minus soak hits | `10` | `10` | yes |
| 137 | nothing is written until the GM commits the card | `true` | `true` | yes |
| 138 | the boxes land on the right monitor | `10` | `10` | yes |
...
| 141 | every one of them is behind the screen and stamped with the chain | `visibility gm, chainId 7a28ab20-3636-4485-9fcb-107cb6e12511` | `attack:gm defense:gm soak:gm` | yes |
| 142 | …and the faces on the record are the faces on the card | `the attack card and its stored row agree, die for die` | `[4,2,6,6,4,1,5,1,3,5,2]` | yes |
...
| 146 | …and ignores the Accuracy limit entirely | `8` | `8` | yes |
| 147 | …and every six rolls again (Rule of Six) | `at least 6 exploded dice` | `1 1 4 2 2 5` | yes |
| 148 | every copilot roll is stamped with the running session | `6 rolls carrying session ff15bece…` | `6/6` | yes |
| 149 | …so they are in the session's own roll log, not floating beside it | `every copilot roll in GET …/rolls?session=` | `6/6` | yes |
| 150 | damage-from-roll is (DV + net) − soak | `9` | `9` | yes |
...
| 153 | Seize the Initiative puts the actor above everyone still in the pass | `strictly above 6` | `8 → 8 (beat 6)` | yes |
...
| 158 | …and the score is base + those five dice + the wound modifier | `8 + 3+4+3+1+3 0 = 22` | `22 (tracker says 22)` | yes |
...
| 161 | every spend announces itself, loudly, by name (FR2.3) | `log lines naming Seize the Initiative and Blitz, with the Edge left` | `Sparrow spends 1 Edge — Seize the Initiative: initiative 8 → 8, ahead of 6 (4/5 left) \| Torque spends 1 Edge — Blitz: 5d6 [3, 4, 3, 1, 3] → initiative 22 (2/3 …` | yes |
...
| 169 | the spike lands as Stun | `8` | `8` | yes |
...
| 179 | the suggestion is logged GM-only, never acted on | `a gm-visibility log line` | `5 line(s), visibility gm` | yes |
...
| | **5b · The shared log** | | | |
| 199 | the log is not empty before the beat starts | `events already on the record` | `160 events, newest id 160` | yes |
| 200 | the roll is on the immutable record | `present in GET /api/campaigns/:id/rolls` | `present` | yes |
| 201 | …and on the shared log the GM reads back | `a roll.created event carrying the roll's id` | `event 161` | yes |
| 202 | …so the log has actually grown since the beat began | `an event id above 160` | `161` | yes |
...
| 204 | …and it reached the live socket as the same event id | `frame id 161` | `161` | yes |
| 205 | table talk from a phone appends to the same log | `event 162 readable back` | `present` | yes |
| 206 | the GM advances the in-game clock | `2076-06-13` | `2076-06-13` | yes |
| 207 | …and the log carries the clock tick as table-visible history (§11) | `a clock.advanced event naming the new date` | `event 163` | yes |
...
| 234 | …whose numbers came off the log, not out of the model (D13) | `the roll tally and the award lines assembled server-side` | `16 public rolls · 3 award line(s) in the body` | yes |
...
| 238 | …the session it was actually about | `ff15bece-78e3-4c77-886c-d307421003f3` | `ff15bece-78e3-4c77-886c-d307421003f3` | yes |
...
| 243 | the session log counted the whole night, copilot dice included | `every persisted roll accounted for by the session` | `31 of 31 persisted rolls, 0 glitches` | yes |
...
| 257 | …and it can still say since when, so the number reads honestly | `the same first-counted timestamp` | `2026-08-29T19:44:14.008Z` | yes |
...
| 265 | the spirit’s spent services and the reagent tin came back too (FR8.3/8.4) | `Ash-of-Kettles at 0 services, 4 drams on the shelf` | `0 service(s) · {"e534083b-d371-495e-8e6b-725fc3c8d049":4}` | yes |
| 266 | and the event log takes the next id, not one already used | `an id above 190` | `event 192` | yes |
